...
def acquisitions_book(
    ...
    Форма по приложению № 2 к Единым правилам организации учёта, хранения и
    использования музейных предметов и музейных коллекций (Приказ Минкультуры
    России № 827 от 23.07.2020). Столбец 9 «№ и дата акта ПХ» оставляется пустым
    для последующего ручного заполнения.
    """
    styles, font_name = _get_styles()
    buffer = io.BytesIO()
